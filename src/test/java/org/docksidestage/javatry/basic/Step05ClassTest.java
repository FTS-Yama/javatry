...
import org.docksidestage.bizfw.basic.buyticket.FewDaysTicket;
...
public class Step05ClassTest extends PlainTestCase {
    ...
    public void test_class_letsFix_makeMethod_twoday() {
        ...
        TicketBooth booth = new TicketBooth();
        int money = 14000;
        TicketBuyResult twoDP = booth.buyTwoDayPassport(money);
        Integer sea = booth.getSalesProceeds() + twoDP.getChange();
    ...
    public void test_class_moreFix_type() {
        ...
        final Ticket buyPassport = booth.buyOneDayPassport(10000);
        if (buyPassport.getDisplayPrice() == 7400) {
            log(buyPassport.getDisplayPrice());
            log("oneDayPassport");
        }
        final TicketBuyResult buyPassport2Result = booth.buyTwoDayPassport(14000);
        final Ticket buyPassport2 = buyPassport2Result.getTicket();
        if (buyPassport2.getDisplayPrice() == 13200) {
            log(buyPassport2.getDisplayPrice());
            log("twoDayPassport");
        }
    ...
    public void test_class_moreFix_wonder() {
        // your confirmation code here
        FewDaysTicket fourDayTicket = new FewDaysTicket(22400, 4);
        log(fourDayTicket.getDisplayPrice());
        for_inPark(fourDayTicket, 4);
    }

    public void for_inPark(final FewDaysTicket fewDayTicket, final int num) {
        for (int i = 0; i < num; i++) {
            fewDayTicket.doInPark();
            log(fewDayTicket.isAlreadyIn());
        }
    }
    ...
    public void test_class_moreFix_yourRefactoring() {
        ...
        //doInParkを確認するメソッド(for_inPark())を作成。
        FewDaysTicket fewDaysTicket = new FewDaysTicket(13200, 3);
        for_inPark(fewDaysTicket, 3);
    }
}
